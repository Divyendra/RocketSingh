...
public abstract class AbstractRouting extends AsyncTask<Void, Void, ArrayList<Route>> {
    ...
    protected ArrayList<Route> doInBackground(Void... voids) {
        try {
            ...
            do {
                parser = new GoogleParser(constructURL());
                routes = parser.parse();
                retryCount++;
            }while (routes != null && routes.size()==0 && parser.getStatus()== GoogleParser.Status.RETRY && retryCount>=3);
            System.out.println("Status: " + parser.getStatus());
            return routes;
        }catch (JSONException e) {
            e.printStackTrace();  //crashlytics
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
